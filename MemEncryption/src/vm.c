...
int mdecrypt(char *virtual_addr) {
  ...
  *pte = *pte | PTE_P;
  ...
  if (potIndex != -1) {
    // set access bit to true
    *pte = *pte | PTE_A;
    // do we need to store this back in the queue???
  	//pgQueue[potIndex] = virtual_addr;
    return 0;
  }
  ...
  while (1) {
    // check if the page on clock handle has access bit and set it to false
    pte_t *currPagepte = walkpgdir(mypd, p->pgQueue[p->clockHand], 0);
...
int getpgtable(struct pt_entry* pt_entries, int num, int wsetOnly) {
  ...
  for (;(int)uva >= 0;uva -=PGSIZE)
  {
  	...
  	//cprintf("The index the page is at is %d\n", bufferContainsPage(curproc, (void *)uva));
  	/*
  	cprintf("uva is %d\n", uva);
